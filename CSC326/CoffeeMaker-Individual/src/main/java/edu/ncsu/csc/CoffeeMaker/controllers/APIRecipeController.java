...
public class APIRecipeController extends APIController {
    ...
    /**
     * REST API method to allow editing a Recipe from CoffeeMaker's Inventory.
     * Converts the JSON RequestBody into a new Recipe and searches for an
     * existing Recipe with the same name, then updates it if found.
     *
     * If a matching Recipe by name is not found, a 404 response is sent.
     *
     * If the Recipe in the RequestBody has 0's for all Ingredient units, or the
     * Recipe has no Ingredients, a 400 response is sent.
     *
     */
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe r ) {
        ...
        if ( r.checkRecipe() || r.getIngredients().size() == 0 ) {
        ...
        return new ResponseEntity( successResponse( r.getName() + " was updated successfully" ), HttpStatus.OK );
    }
}
