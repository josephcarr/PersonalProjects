...
public class RegistrationManagerTest {
	...
	public void testGetCourseCatalog() {
		...
		cc.loadCoursesFromFile(validTestFileCourse);
	...
	public void testGetStudentDirectory() {
		...
		//Loads students, gets directory length, makes new directory, gets new directory length
		sd.loadStudentsFromFile(validTestFileStudent);
		...
		assertTrue(sd.removeStudent(ID));
		assertEquals(0, sd.getStudentDirectory().length);
	}
	...
	public void testLogin() {		
		manager.getStudentDirectory().addStudent("John", "Smith", "jsmith", "jsmith@ncsu.edu", "qwerty", "qwerty", 10);
		assertTrue(manager.login("jsmith", "qwerty"));
		User u1 = manager.getCurrentUser();
		assertEquals("John", u1.getFirstName());
		assertEquals("Smith", u1.getLastName());
		assertEquals("jsmith", u1.getId());
		assertEquals("jsmith@ncsu.edu", u1.getEmail());
		
		manager.logout();
		
		assertThrows(IllegalArgumentException.class, () -> manager.login("oput", "qwerty"));
		assertFalse(manager.login("jsmith", "poiuy"));

		assertTrue(manager.login(prop.getProperty("id"), prop.getProperty("pw")));

		User u2 = manager.getCurrentUser();
		assertEquals(prop.getProperty("first"), u2.getFirstName());
		assertEquals(prop.getProperty("last"), u2.getLastName());
		assertEquals(prop.getProperty("id"), u2.getId());
		assertEquals(prop.getProperty("email"), u2.getEmail());
	}
	...
	public void testEnrollStudentInCourse() {
		...
		try {
			...
			assertEquals(registrarUsername, manager.getCurrentUser().getId(), "RegistrationManager.enrollStudentInCourse() - currentUser is registrar");
		}
		manager.logout();
	...
	public void testAddFacultyToCourse() {
		...
		Faculty f = new Faculty("first", "last", "id", "email@ncsu.edu", "hashedpassword", 2);
		
		manager.login(registrarUsername, registrarPassword);
		
		assertTrue(manager.addFacultyToCourse(catalog.getCourseFromCatalog("CSC216", "001"), f));
		assertTrue(manager.addFacultyToCourse(catalog.getCourseFromCatalog("CSC226", "001"), f));
	...
	public void testResetFacultySchedule() {
		...
		assertTrue(manager.resetFacultySchedule(f));
