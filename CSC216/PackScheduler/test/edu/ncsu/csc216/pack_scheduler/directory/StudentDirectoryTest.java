...
public class StudentDirectoryTest {
	...
	/**
	 * Tests StudentDirectory.addStudent() where the invalid values should
	...
	public void testAddStudent() {
		...
		//Attempt to add Student with empty password
		try {
			sd.addStudent("Casey", "Dockery", "cbdocke2", "cbdocke2@ncsu.edu", "", "", MAX_CREDITS);
			fail();
		} catch (IllegalArgumentException e) {
			assertEquals("Invalid password", e.getMessage());
		}
		//Attempt to add Student with password that are not equal
		try {
			sd.addStudent("Doug", "Cicin", "dccicin", "dccicin@ncsu.edu", "badpassword", "password", MAX_CREDITS);
			fail();
		} catch (IllegalArgumentException e) {
	 ...
	 */
	@Test
	public void checkGetStudentById() {
		StudentDirectory sd = new StudentDirectory();
		
		sd.loadStudentsFromFile(validTestFile);
		Student s1 = sd.getStudentById("lberg");
		assertEquals("Lane", s1.getFirstName());
		assertEquals("Berg", s1.getLastName());
		assertEquals("lberg", s1.getId());
	}
